...
safe_register(Tenant, TenantAdmin)
...
safe_register(Branch, BranchAdmin)

# ============================================================
#   🔹 FINANCIAL YEAR & REPORTING PERIOD ADMIN
# ============================================================
...
safe_register(FinancialYear, FinancialYearAdmin)
...
safe_register(ReportingPeriod, ReportingPeriodAdmin)
...
safe_register(Holiday, HolidayAdmin)
...
class TenantRuleConfigForm(forms.ModelForm):
    ...
    # Workflow
    workflow_approval_levels = forms.CharField(required=False, label="Approval Levels (comma separated)")
    workflow_approver_roles = forms.CharField(required=False, label="Approver Roles (comma separated)")
    workflow_rejector_roles = forms.CharField(required=False, label="Rejector Roles (comma separated)")
    workflow_auto_validation = forms.BooleanField(required=False, label="Auto Document Validation")
    workflow_upload_limit = forms.IntegerField(required=False, label="Document Upload Limit (MB)")
...
class TenantRuleConfigAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Access → Leads Permissions", {"classes": ("collapse",), "fields": ("leads_view", "leads_add", "leads_edit", "leads_delete")}),
        ("Access → Module Access", {"classes": ("collapse",), "fields": ("module_crm", "module_loan", "module_collection")}),
        ("Workflow Configuration", {"classes": ("collapse",), "fields": ("workflow_approval_levels", "workflow_approver_roles", "workflow_rejector_roles", "workflow_auto_validation", "workflow_upload_limit")}),
        ("Validation Rules", {"classes": ("collapse",), "fields": ("val_unique_email", "val_pan_format", "val_aadhaar_format", "val_phone_10")}),
        ("Assignment Rules", {"classes": ("collapse",), "fields": ("assign_lead_category", "assign_auto_sales", "assign_auto_verification", "assign_auto_credit")}),
        ("Security Rules", {"classes": ("collapse",), "fields": ("sec_password_min", "sec_password_special", "sec_timeout")}),
        ("Generated JSON (Readonly)", {"classes": ("collapse",), "fields": ("generated_json",)}),
    )
...
safe_register(TenantRuleConfig, TenantRuleConfigAdmin)
